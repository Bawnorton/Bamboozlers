...
public class GroupChatTests : AuthenticatedBlazoriseTestBase
{
    ...
    private Task<(List<User>, List<Friendship>, List<GroupChat>, List<GroupInvite>)> BuildTestCases()
    {
        List<User> testUsers = [];
        ...
        return Task.FromResult<(List<User>, List<Friendship>, List<GroupChat>, List<GroupInvite>)>((
            testUsers,
            testFriendships,
    ...
    private static Task<bool> CompAddMember_CheckInviteListEntry(
        User self,
        User friend,
        Chat chat,
    ...
    public async void GroupChatTests_CompAddMember()
    {
        ...
        {
            Assert.True(await CompAddMember_CheckInviteListEntry(subjectUser, friend, subjectGroup, testInvites,
                component));
        }
        ...
        foreach (var friend in friends)
        {
            Assert.False(await CompAddMember_CheckInviteListEntry(subjectUser, friend, subjectGroup, testInvites,
                component));
        }
        ...
        IRenderedFragment fragment)
    {
        foreach (var member in subjectGroup.Users)
        {
            ...
            {
                if (member.Id == subjectUser.Id) continue;
    ...
    public async void GroupChatTests_CompChatSettings()
    {
        ...
        component.Dispose();
        Ctx.RenderComponent<CompChatSettings>(
            parameters
                => parameters.Add(p => p.ChatID, subjectGroup.ID)
        );
    }
