...
public class MockUserManager
{
    ...
    public MockUserManager(TestContextBase ctx, MockDatabaseProvider mockDatabaseProvider)
    {
        ...
        _mockUserManager.Setup(x
            ...
            => claimsPrincipal.Identity is null
                ? null
                : _mockDatabaseProvider.GetDbContextFactory().CreateDbContext().Users
                    .FirstOrDefault(u => u.UserName == claimsPrincipal.Identity.Name)
        );
        ...
        _mockUserManager.Setup(x
            ...
            => userId is null
                ? null
                : _mockDatabaseProvider.GetDbContextFactory().CreateDbContext().Users
                    .FirstOrDefault(m => m.Id == int.Parse(userId))
        );
            ...
            =>
        {
            if (password.IsNullOrEmpty())
                return IdentityResult.Failed(
                    [new IdentityError { Description = "Password entered was null or empty." }]);
            return password.Equals(user.PasswordHash)
                ? IdentityResult.Success
                : IdentityResult.Failed([new IdentityError { Description = "Password does not match." }]);
        });
            ...
            =>
        {
            ...
            return _mockDatabaseProvider.GetDbContextFactory().CreateDbContext().Users
                .FirstOrDefault(m => m.Email == newEmail) is null
                ? IdentityResult.Success
            ...
            =>
        {
            if (newUsername.IsNullOrEmpty())
                return IdentityResult.Failed(
                    [new IdentityError { Description = "Username entered was null or empty." }]);
            return _mockDatabaseProvider.GetDbContextFactory().CreateDbContext().Users
                .FirstOrDefault(m => m.UserName == newUsername) is null
                ? IdentityResult.Success
    ...
    public User CreateMockUser(
        ...
        byte[]? avatar = null)
    {
        var userList = _mockDatabaseProvider.GetMockAppDbContext().MockUsers.MockUsersSet.Object.ToList();
