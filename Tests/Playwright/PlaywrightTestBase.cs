...
public class PlaywrightTestBase : IClassFixture<CustomWebApplicationFactory>
{
    ...
    protected async Task Login(IPage page, string username = TestUserName, string email = TestUserEmail,
    ...
    protected async Task<IPage> SetupAndLogin(bool headless = true, string username = TestUserName,
        string email = TestUserEmail, string password = TestUserPassword)
    {
    ...
    protected async Task<IPage> SetupAndLoginAtFirstDm(bool headless = true, string username = TestUserName,
        string email = TestUserEmail, string password = TestUserPassword)
    {
    ...
    protected async Task CreateDm(User owner, User other)
    {
        ...
        var contextFactory = scope.ServiceProvider.GetRequiredService<IDbContextFactory<AppDbContext>>();
        await using var context = await contextFactory.CreateDbContextAsync();
        var chat = await context.Chats.FirstOrDefaultAsync(chat =>
            chat.Users.Contains(owner) && chat.Users.Contains(other) && chat.Users.Count == 2);
        if (chat != null)
            throw new InvalidOperationException(
                $"Direct message already exists between {owner.UserName} and {other.UserName}");
